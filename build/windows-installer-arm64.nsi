; 幻梦P2P Windows ARM64 安装程序
; 功能：自定义安装路径、记住上次路径、WebView2集成、自动更新支持
; ARM64 专用脚本：安装节额外携带 wintun.dll 与 WebView2Loader.dll，
; 与应用 exe 同级安装——否则装完缺 DLL，TUN 虚拟网卡无法启动。
; 用法: makensis -INPUTCHARSET UTF8 -DSTAGING_DIR=/path -DOUTPUT_DIR=/path windows-installer-arm64.nsi
; 注意：此脚本与 x64 的 windows-installer.nsi 相互独立，改动请勿交叉。
...
OutFile "${OUTPUT_DIR}\${APP_NAME}_${APP_VERSION}_arm64-setup.exe"
...
Section "Main" SEC_MAIN
  ...
  ; ARM64 必需的同级运行时 DLL——缺 wintun.dll 则 TUN 无法启动，
  ; 缺 WebView2Loader.dll 则前端渲染失败。此二文件由 ARM64 构建 job
  ; 下载并放入 STAGING_DIR，与 exe 一同打进安装器。
  File "${STAGING_DIR}\wintun.dll"
  File "${STAGING_DIR}\WebView2Loader.dll"
